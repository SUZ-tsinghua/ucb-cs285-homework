...
class PGAgent(nn.Module):
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        discounted_factors = self.gamma ** np.arange(len_rewards)
        discounted_return = np.full(
            (len_rewards,), np.sum(rewards * discounted_factors)
        )
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        """
        rtg: np.ndarray = np.zeros(len(rewards))
        for i in reversed(range(len(rewards))):
            if i == len(rewards) - 1:
                rtg[i] = rewards[i]
            else:
                rtg[i] = rtg[i + 1] * self.gamma + rewards[i]
